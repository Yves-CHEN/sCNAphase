...
//extern  "C"   void countReads(char* bamF, char* chrID, int* start, int* windowSize, int* num, int* qcThresh, int* countVec);
    //extern "C" void countReads(char** bamF, char** chrID, int* start, int* windowSize, int* num, int* qcThresh, int* countVec);
    //
extern void calcLikelihood ( int* depths, int* seq_len, int* genotypes, int* _k,
...
//
        ...
        while(iter < *maxiter)
        {
            ...
            if(iter  == 0)
            {
                ...
                getTC_Ratio_2(filter, depths, genotypes, i_tpm, pi, &T, &k, 
                    ...
                    DOA_range,  DOA,
                    del_range,  &deletion,  ifSigAmplified,  0, *underate);
            }
            else
            {
                //local search
    ...
    ///               g_rcov);
            ...
            emissionDist(depths, T, genotypes, k, cellularity_iter, *DOA, yll, deletion, *underate);
            bool faster = false;
